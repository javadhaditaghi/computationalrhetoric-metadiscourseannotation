...
class GeminiMetadiscourseAnnotator:
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-3-pro-preview",
        ...
        Args:
            api_key: Google Gemini API key. If None, will look for GEMINI_API_KEY in .env file
            model: Gemini model to use for annotation (gemini-1.5-pro, gemini-1.5-flash)
        ...
        if not self.api_key:
            raise ValueError("API key must be provided either as parameter or GEMINI_API_KEY in .env file")

        # Configure Gemini
        genai.configure(api_key=self.api_key)

        # Store model name and context settings
        ...
        # NOW initialize model with system instruction
        ...
        self.logger.info(f"Model initialized with system instruction including framework")
    ...
    def _create_system_instruction(self) -> str:
        """
        Create system instruction for Gemini that includes the JSON framework.

        Returns:
            System instruction string with the complete framework
        """
        system_instruction = f"""You are a linguistic analyst specializing in metadiscourse classification.
    ...
    def _call_gemini(self, prompt: str, max_retries: int = 5, initial_delay: float = 2.0) -> Optional[str]:
        """
        Call Gemini API with improved retry logic for handling rate limits and errors.
            ...
            try:
                ...
                if response.candidates and hasattr(response.candidates[0], 'finish_reason'):
                    ...
                    if finish_reason and 'SAFETY' in str(finish_reason):
                        self.logger.warning(f"Response blocked by safety filters (attempt {attempt + 1}/{max_retries})")
                        if attempt < max_retries - 1:
                            time.sleep(initial_delay)
                        continue

                # Check if response has text and log token usage
                if response.text:
                    # Log token usage if available
                    if hasattr(response, 'usage_metadata'):
                        usage = response.usage_metadata
                        self.logger.debug(
                            f"Tokens - Input: {usage.prompt_token_count}, Output: {usage.candidates_token_count}, Total: {usage.total_token_count}")
                    return response.text
                else:
                    self.logger.warning(f"Empty response received (attempt {attempt + 1}/{max_retries})")
                    if attempt < max_retries - 1:
                        time.sleep(initial_delay)
                    continue
                ...
                if "blocked" in error_str or "safety" in error_str:
                    self.logger.warning(f"Prompt blocked by safety filters (attempt {attempt + 1}/{max_retries})")
                    if attempt < max_retries - 1:
                        time.sleep(initial_delay)
                    else:
                        self.logger.error("Prompt consistently blocked by safety filters")
                        return None
                ...
                elif "overloaded" in error_str or "503" in error_str or "500" in error_str:
                    if attempt < max_retries - 1:
    ...
    def annotate_batch(self, df: pd.DataFrame, batch_size: int = 10, output_file: str = None,
        ...
        for idx, row in df.iterrows():
            ...
            # Create prompt with thesis_code, section
    ...
    def save_results(self, df: pd.DataFrame, output_file: str = "annotated_metadiscourse.csv"):
        ...
        """
        # Ensure the result/gemini directory exists
        output_path = Path(output_file)
        result_dir = Path("result/gemini")
        result_dir.mkdir(parents=True, exist_ok=True)

        # Update output path to be in result/gemini directory
        ...
        summary = {
            ...
            'annotation_model': self.model_name  # Add model info to summary
        }
...
def main(specific_file: Optional[str] = None, context_before: Optional[int] = None,
    ...
    """
    try:
        annotator = GeminiMetadiscourseAnnotator(
        ...
        model_short = annotator.model_name.replace("gemini-", "").replace("-", "_").replace(".", "_")
        ...
        print(f"Results saved to: result/gemini/{output_file}")
        print(f"Annotation model: {annotator.model_name}")
...
if __name__ == "__main__":
    ...
    # python gemini_metadiscourse_annotator.py test_restricted.csv --rows=2 --context-before=30 --context-after=30