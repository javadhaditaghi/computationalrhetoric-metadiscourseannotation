...
class DeepSeekMetadiscourseAnnotator:
    ...
    def _create_system_message(self) -> str:
        """
        Create system message for DeepSeek that includes the JSON framework.

        Returns:
            System message string with the complete framework
        """
        system_message = f"""You are a linguistic analyst specializing in metadiscourse classification.
...
Analyze texts according to this framework and provide classifications in the exact JSON format specified in the "output_format" section."""
    ...
    def _create_annotation_prompt(self, sentence: str, context_before: str, context_after: str,
        ...
        """
        Create the user prompt for annotation.
        ...
        # Create the user prompt (similar to Gemini)
...
Provide your classification in JSON format."""
    ...
    def _call_deepseek(self, user_prompt: str, max_retries: int = 5, initial_delay: float = 2.0) -> Optional[str]:
        """
        Call DeepSeek API with improved retry logic for handling overloaded servers.

        Args:
            user_prompt: The user prompt to send to DeepSeek
        ...
        # Create system message with framework
...
if __name__ == "__main__":
    ...
    # python deepseek_metadiscourse_annotator_json.py test_restricted.csv --rows=2 --context-before=30 --context-after=30