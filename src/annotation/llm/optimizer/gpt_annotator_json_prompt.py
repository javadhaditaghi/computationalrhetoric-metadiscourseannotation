...
class AnnotationOptimizer:
    ...
    def _load_minimal_adjudication_prompt(self) -> str:
        ...
        try:
            prompt_path = os.path.join(project_root, Config.MINIMAL_ADJUDICATION_PROMPT_PATH)
            with open(prompt_path, 'r', encoding='utf-8') as f:
                return f.read()
    ...
    def _normalize_expression(self, expr: str) -> str:
        ...
        - Unicode characters (accents, special chars → ASCII)
        ...
        # Handle Unicode (accents, special characters → ASCII)
    ...
    def test_normalization_with_real_data(self, max_cases: int = 10):
        ...
        try:
            ...
            stats = {
                'total_cases': 0,
                'boundary_disagreements_before': 0,
                'boundary_disagreements_after': 0,
            ...
            for idx, row in merged_df.iterrows():
                ...
                # TEST BOUNDARY NORMALIZATION - Compare ALL expressions
                ...
                if max_expr_count > 1:
                    print(f"ℹ️  Note: This case has up to {max_expr_count} expressions. Comparing all.")
                    print()

                # Compare each expression position
                for expr_num in range(1, max_expr_count + 1):
                    if max_expr_count > 1:
                        print(f"  Expression {expr_num}:")
                        print("  " + "-" * 76)

                    # Get expressions at this position
                    expressions = [self._get_expression(ann, expr_num) for ann in annotations]

                    # Check if at least 2 models have this expression
                    non_empty_count = sum(1 for e in expressions if e)

                    if non_empty_count < 2:
                        if max_expr_count > 1:
                            print(f"    ℹ️  Skipped: Less than 2 models have expression_{expr_num}")
                        continue
                    ...
                    for name, expr, count in zip(model_names, expressions, expression_counts):
                        if expr:  # Only show non-empty
                            prefix = "    " if max_expr_count > 1 else ""
                            expr_label = f" (has {count} expr)" if count > 1 and expr_num == 1 else ""
                            print(f"{prefix}  {name:10s}: '{expr}'{expr_label}")
                    ...
                    for name, orig, norm in zip(model_names, expressions, normalized_expressions):
                        if norm:  # Only show non-empty
                            prefix = "    " if max_expr_count > 1 else ""
                            changed = " ✓ (changed)" if orig != norm else ""
                            print(f"{prefix}  {name:10s}: '{norm}'{changed}")

                    # Check for disagreement
                    unique_before = set([e for e in expressions if e])
                    unique_after = set([e for e in normalized_expressions if e])

                    has_disagreement_before = len(unique_before) > 1
                    has_disagreement_after = len(unique_after) > 1
                    ...
                    if has_disagreement_before and not has_disagreement_after:
                        print(f"{prefix}  ✅ FALSE POSITIVE PREVENTED - Normalization fixed formatting difference")
                        stats['false_positives_prevented'] += 1
                        if expr_num == 1:  # Only count once for statistics
                            stats['boundary_disagreements_before'] += 1
                    elif has_disagreement_before and has_disagreement_after:
                        print(f"{prefix}  ⚠️  REAL DISAGREEMENT - Models chose different boundaries")
                        if expr_num == 1:  # Only count once for statistics
                            stats['boundary_disagreements_before'] += 1
                            stats['boundary_disagreements_after'] += 1
                    else:
                        print(f"{prefix}  ✅ ALL AGREE - No disagreement")

                    if max_expr_count > 1:
                        print()  # Extra spacing between expressions

                print()

                # After the boundary comparison section, add this:
            ...
            print(f"   Before normalization: {stats['boundary_disagreements_before']}")
            print(f"   After normalization:  {stats['boundary_disagreements_after']}")
            print(
                f"   False positives prevented: {stats['boundary_disagreements_before'] - stats['boundary_disagreements_after']}")
            ...
            if stats['false_positives_prevented'] > 0:
                total_disagreements_before = (stats['boundary_disagreements_before'] +
                                              stats['scope_disagreements_before'] +
...
#     # Initialize optimizer with retrieval enabled
#     optimizer = AnnotationOptimizer(
#         openai_api_key=Config.OPENAI_API_KEY,
#         model=Config.OPENAI_MODEL,
#         use_retrieval=True  # Enable smart retrieval for token savings
#     )
#
#     # Run full pipeline
#     try:
