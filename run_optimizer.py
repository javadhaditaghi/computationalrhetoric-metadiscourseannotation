...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Run annotation optimization with configurable context words')

    parser.add_argument(
        '--context-before',
        type=int,
        default=10,
        help='Number of words to include from context_before column (default: 10)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Number of words to include from context_after column (default: 10)'
    )
    ...
    parser.add_argument(
        ...
        help=f'Base path for data files (default: {Config.BASE_DATA_PATH})'
    )
...
def main():
    ...
    try:
        # Initialize optimizer with context word parameters
        logger.info("Initializing annotation optimizer...")
        optimizer = AnnotationOptimizer(
            openai_api_key=Config.OPENAI_API_KEY,
            model=args.model,
